...
try:
    ...
    cloudflare_dns_prefix = config.get('cloudflare', 'cloudflare_dns_prefix')
...
if vms:
    for vm in vms:
        try:
            if cloudflare_dns_prefix:
                cf.update_record(f"{vm['name']}.{cloudflare_dns_prefix}.{cloudflare_zone}", vm['ip_address'])
            else:
